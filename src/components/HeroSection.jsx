    ...
    <section className="relative w-full overflow-hidden" style={{ height: "100vh", minHeight: 500 }}>
      ...
      <div className="absolute inset-0 flex">
        {panels.map((panel, i) => (
          <motion.div
            ...
            style={{ flex: 1, transition: "flex 0.5s cubic-bezier(0.4,0,0.2,1)" }}
            onMouseEnter={(e) => (e.currentTarget.style.flex = "1.8")}
            onMouseLeave={(e) => (e.currentTarget.style.flex = "1")}
          >
            <img src={panel.img} alt={panel.label} className="w-full h-full object-cover" loading={i === 0 ? "eager" : "lazy"} />
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/30 to-black/40" />
            <div className="absolute bottom-6 left-0 right-0 text-center">
              <span className="font-body text-[9px] font-semibold tracking-[0.25em] text-white/60 uppercase">{panel.label}</span>
            </div>
          </motion.div>
        ))}
      </div>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center z-10 px-4 text-center">
        ...
        >
          ...
          <span className="font-body text-[10px] tracking-[0.4em] text-white/50 uppercase">International</span>
          <div className="h-px w-12 bg-white/30" />
        </motion.div>
      </div>
      ...
      >
        ...
        <span className="font-body text-[9px] tracking-[0.35em] text-white/50 uppercase">Scroll</span>
      </motion.button>
    </section>
  );
}